...
function wpcf7_text_form_tag_handler( $tag ) {
	...
	$inputname = get_field('nadpis_vashe_imya','options');
	$inputphone = get_field('nadpis_telefon','options');
	$inputemail = get_field('nadpis_email','options');

    $currenttype = $tag->name;
    if ($currenttype == "name-input"){
        $addtext = $inputname;
	...
	$html = sprintf(
		'<span data-currenttext="' . $addtext . '" class="wpcf7-form-control-wrap %1$s"><input %2$s />%3$s</span>',
